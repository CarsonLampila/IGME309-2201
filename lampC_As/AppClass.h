...
#pragma endregion
	...
	/*
	USAGE: A* find path between start and target pos
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void PathFollowing();
	...
	/*
	USAGE: Calc distance between current and target tiles
	ARGUMENTS: Current X, Current Y, Target X, Target Y
	OUTPUT: Calculated Distance
	*/
	int ManhattenDistance(int moveX, int moveY, int targetX, int targetY);

	/*
	USAGE: Moves body along A* path
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void FollowPath(MyRigidBody* body);

	/*
	USAGE: Randomly finds new viable position for target
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void NewTarget();
	...
	/*
	USAGE: Removes A* path and grid parents + distances
	ARGUMENTS: ---
	OUTPUT: ---
	*/
	void Clean();
