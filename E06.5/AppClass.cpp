...
void Application::Display(void)
{
	...
	// Define Current
	current = m_stopsList[stopNum];
	// Define Next as following stop and loop back to start
	if (destination >= m_stopsList.size())
	{
		destination = 0;
	}
	next = m_stopsList[destination];

	// Define times
	float arriveTime;
	// Regular
	if (stopNum != m_stopsList.size() - 1)
	{
		arriveTime = 3.0;
	}
	// End - Start
	...
	if (timePercent >= 1.0f)
	{
		...
		// Reset stopNum at end
		if (stopNum >= m_stopsList.size())
		{
			stopNum = 0;
		}
	}
